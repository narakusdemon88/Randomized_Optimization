"""
N Queen Implementation for Assignment 2
...
def plot_results(
    ...
    if save_results:
        plt.savefig(f"../figures/N Queens/{title}.png")
    plt.show()
    plt.clf()
...
class GeneticAlgorithm(ROParent):
    ...
    def process(self, p_len, seed=909):
        ...
        _, fitness, curve = mlh.genetic_alg(problem,
                                            ...
                                            max_attempts=self.maximum_attempt,
                                            pop_size=int(population_size[0]))
        t2 = time.perf_counter()
...
class SimulatedAnnealing(ROParent):
    def __init__(self, fitness):
        super().__init__(fitness=fitness)
        self.schedule = mlh.ExpDecay(init_temp=2_000, exp_const=.95)
...
def main():
    ...
    # Create a p_len range to iterate through
    ...
    for graph in graphs:
        plot_results(
            ...
            title=f"{graph['title']} versus p_len",
